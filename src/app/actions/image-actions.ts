...
export async function storeImages(data: storeImageInput[] ) {
    ...
    for(const img of data){
        ...
        const{error : storageError} = await supabase.storage.from("generated_images").upload(filePath,arrayBuffer,{contentType: `image/${type}`,cacheControl:"3600",upsert:false})
         if(storageError){
            uploadResults.push({
                fileName,
...
export async function deleteImage(id: string, imageName:string) {
    ...
    // deleting image from the table / db . 
  const {data,error} = await supabase.from("generated_images").delete().eq("id",id);
    if(error){
      ...
      // delete the image from the supabase storage also.
      const {} = await supabase.storage.from("generated_images").remove([`${user.id}/${imageName}`])
